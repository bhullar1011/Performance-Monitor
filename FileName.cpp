...
public:
    NetworkUsage(HANDLE process) : process(process) {}

    ~NetworkUsage() {
        CloseHandle(process);
    }

    void print_network_usage() const {
        IO_COUNTERS io_counters;
        if (GetProcessIoCounters(process, &io_counters)) {
            std::cout << "Network usage: " << io_counters.OtherTransferCount / 1024 << " KB" << std::endl;
        }
        else {
            std::cerr << "Error getting network usage. Error code: " << GetLastError() << std::endl;
        }
    }
};
int main()
{
    ...
    NetworkUsage network_usage(handle);
    ...
    while (true) {
        ...
        network_usage.print_network_usage();
